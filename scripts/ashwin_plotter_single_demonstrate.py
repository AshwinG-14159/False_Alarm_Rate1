...
for row in set_of_rows:
    ...
    for binning_id in range(len(binnings)):
        ...
        try:
            result_file = np.load(f'{result_path}/{orbit}/{orbit}_{binning}_nsigma_FAR_quadrature.npy')
        except:
            print("file", f'{result_path}/{orbit}/{orbit}_{binning}_nsigma_FAR_quadrature.npy', "not found")
            continue
        if(result_file.shape!=(2,62)):
            print("wrong shape")
            continue
        ...
        if(threshs.shape!=(3,61) or counts.shape!=(3,61) or datapoints.shape!=(3,)):
...
for binning in binnings:
    my_data = np.load(f'{result_path}/{target_orbit}/{target_orbit}_{binning}_nsigma_FAR_quadrature.npy')
    list_of_thresh = my_data[0][1:]
    list_of_counts = my_data[1][1:]
    thresh = list_of_thresh[np.where(list_of_counts!=0)][-1]
    print(thresh)
    # print(my_data)
    # print(my_data.shape)
    plt.axvline(x=thresh, color=f'C{t}', linestyle='--', label = f"GRB:{binning}", alpha = 0.3)
    t+=1

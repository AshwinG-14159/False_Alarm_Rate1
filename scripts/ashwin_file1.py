...
for row in set_of_rows:
    ...
    with fits.open(mkf_file) as hdul: #get the pointing directions
        RA_pnt = f"{hdul[0].header['RA_PNT']}d"
        dec_pnt = f"{hdul[0].header['DEC_PNT']}d"
    # print(RA_pnt, dec_pnt)
    try:
        RA_pnt = Angle(RA_pnt)
    except u.UnitsError:
        RA_pnt = Angle(RA_pnt, unit=u.deg)

    try:
        dec_pnt = Angle(dec_pnt)
    except u.UnitsError:
        dec_pnt = Angle(dec_pnt, unit=u.deg)
    
    try:
        transient = SkyCoord(RA_pnt.deg * u.deg, dec_pnt.deg * u.deg)

        #Earth:
        earthx = mkfdata['posx'] * u.km
        earthy = mkfdata['posy'] * u.km
        earthz = mkfdata['posz'] * u.km
        earth = SkyCoord(-earthx, -earthy, -earthz, frame='icrs', representation_type='cartesian')

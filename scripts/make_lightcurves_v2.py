...
def pixclean(quad_dsevt, bclive_tfile, maxpixcount, maxdetcount, outpath):
        ...
        print('------------------------------------------------','\r\n')
        # Run cztpixclean
        subprocess.call(['cztpixclean', 'infile=' + quad_dsevt,
                                         'inlivetimefile=' + bclive_tfile,
                                         'outfile1=' + quad_pcevt,
                                         'outfile2=' + quad_dblevt,
                                         'outlivetimefile=' + quad_livet,
                                         'badpixfile=' + quad_badpix, 'nsigma=5',
                                         'det_tbinsize=1', 'pix_tbinsize=1',
                                         'det_count_thresh=' + str(maxdetcount),
                                         'pix_count_thresh=' + str(maxpixcount),
                                         'writedblevt=yes'])
...
def bindata(quad_cleanevt, mkffile, flag_badpix, quad_livet, tbin, outbase, band):
        ...
        subprocess.call(['cztbindata', 'inevtfile=' + quad_cleanevt,
                                         ...
                                         'emin=' + e_min,'emax='+e_max, 'generate_eventfile=yes',
...
def getbin_lcs(tbin, quad_lc, threshold, filtertype, filterorder,
        ...
        right_indices = []
        for i in range(1):
                loc = np.where(np.diff(zero_runs(lc_all[i])) >= 1000/tbin)[0]
                locc = np.diff(zero_runs(lc_all[i]))[loc] ## the lengths of all zeros found
                if len(loc) >= 1:
                        for k in range(len(loc)):
                                left_indices.append(zero_runs(lc_all[i])[loc][k][0])
                                right_indices.append(zero_runs(lc_all[i])[loc][k][1])
        ...
        if left_indices:
                for left_index in left_indices:
                        saa_start.append(bins_all[0][left_index] - t_buffer)
        if right_indices:
                for right_index in right_indices:
                        saa_end.append(bins_all[0][right_index] + t_buffer) 
...
def get_vetocount(orbit):
        ...
        if(quad_clean==[]):
                ...
                veto_lc = np.array(veto_lc,dtype=object)
                zeros = []
                new_time=[]
                new_veto_lc_all=[]
